...
import { Plus, Trash2, Star, ChevronDown, ChevronUp, X } from "lucide-react";
import { useState } from "react";
import type { FunnelSection, SectionItem, PricingPlanItem } from "@/lib/funnels/types";
            ...
            <div key={idx} className="rounded-md border border-white/10 bg-zinc-950/60">
              <div className="flex items-center gap-2 px-2 py-2">
                <button
                  ...
                  className={`${
                    item.data.highlighted ? "text-amber-300" : "text-white/30 hover:text-white/60"
                  }`}
                  title={item.data.highlighted ? "Plan mis en avant" : "Mettre en avant"}
                >
                  <Star className="h-4 w-4" fill={item.data.highlighted ? "currentColor" : "none"} />
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  {item.data.price && (
                    <span className="ml-2 text-xs text-white/50">— {item.data.price}</span>
                  )}
                </button>
                <button
                  type="button"
